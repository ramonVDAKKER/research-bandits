...
# App Service Plan (Production tier, Linux containers)
module "app_service_plan" {
  source = "../../modules/app_service_plan"

  name                = "asp-${var.project_name}-prd"
  location            = var.location
  resource_group_name = azurerm_resource_group.main.name
...
# Azure Container Registry (Standard/Premium SKU with RBAC)
...
module "backend_job" {
  ...
  name                          = "caj-${var.project_name}-backend-prd"
  location                      = var.location
  resource_group_name           = azurerm_resource_group.main.name
  container_apps_environment_id = module.container_apps_environment.id
  container_image               = "${module.container_registry.login_server}/research-bandits-backend:prd-latest"
...
module "frontend_app" {
  ...
  name                = "app-${var.project_name}-frontend-prd"
  location            = var.location
  resource_group_name = azurerm_resource_group.main.name
  app_service_plan_id = module.app_service_plan.id
  container_image     = "research-bandits-frontend:prd-latest"
  acr_login_server    = module.container_registry.login_server
